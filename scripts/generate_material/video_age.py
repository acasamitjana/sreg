...
def compute_frames(data_list, T, ages, framedir, interp_method):
    def function(v_age_i, it_v_age_i):
        ...
        if i_left == i_right:
            ...
            w_left = 1 - w_right
            R = -T[i_left] + T[i_right]
            R_F = integrate_NR(-R, image_shape=image_shape, int_end=w_right)
            R_B = integrate_NR(R, image_shape=image_shape, int_end=w_left)

            del R

            I_left = deform3D(data_list[i_left], R_F, interp_method)
            I_right = deform3D(data_list[i_right], R_B, interp_method)
...
OUTPUT_DIR = join(config_data.ALGORITHM_DIR, 'ST_RegNet', 'l1')

arg_parser = ArgumentParser(description='Computes the video for several subjects')
arg_parser.add_argument('--subject')
...
if not exists(subject.template):
    print("No linear template found for subject " + str(subject.id))
    exit()

if len(subject.slice_list) == 1:
    print("Subject " + str(subject.id) + ' has a single timepoint and hence not processed.')
    exit()
...
tp_list = subject.sid_list
tp_list.sort(key=lambda x: int(x))
for timep_id in tp_list:
    print(timep_id)
    timep = subject.slice_dict[timep_id]
    ages.append(timep.demodict['AGE'])

    if not exists(join(OUTPUT_DIR, subject.id, timep_id + '.field_z.nii.gz')):
        print("No deformation files found for subject " + str(subject.id))
        exit()
    ...
    proxy = nib.load(join(OUTPUT_DIR, subject.id, timep_id + '.field_x.nii.gz'))
    svfx = np.asarray(proxy.dataobj)
    proxy = nib.load(join(OUTPUT_DIR, subject.id, timep_id + '.field_y.nii.gz'))
    svfy = np.asarray(proxy.dataobj)
    proxy = nib.load(join(OUTPUT_DIR, subject.id, timep_id + '.field_z.nii.gz'))
...
if SAVE_VIDEO:
    for it_v_age_i, v_age_i in enumerate(v_ages):
        ...
        image = np.clip(image*1.5,0, 255).astype('uint8')
        video.write(image)
        if it_v_age_i == 0:
