...
for it_subject, subject in enumerate(subject_list):
    ...
    for it_tp, tp in enumerate(timepoints):
        if not exists(join(results_dir_sbj, tp.id + '.flow.nii.gz')):
            ...
            flow = algorithm_utils.integrate_NR(svf, subject_shape)
    ...
    for it_tp, tp in enumerate(timepoints):
        ...
        if DEBUG:
            ...
            nib.save(img, tp.get_filepath('nonlinear_mask_' + reg_algorithm))
            del mask

            img = nib.Nifti1Image(im_resampled[1], subject.vox2ras0)
            nib.save(img, tp.get_filepath('nonlinear_mask_dilated_' + reg_algorithm))
        ...
        nib.save(img, tp.get_filepath('nonlinear_resample_' + reg_algorithm))
        del mri
        ...
        nib.save(img, tp.get_filepath('nonlinear_seg_' + reg_algorithm))
        del seg
    ...
    for it_tp, tp in enumerate(timepoints):
        ...
        nib.save(img, tp.get_filepath('nonlinear_image_' + reg_algorithm))
