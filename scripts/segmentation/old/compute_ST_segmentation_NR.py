...
def label_fusion(subject):
    ...
    if not exists(join(flow_dir, timepoints[0].id + '.svf.nii.gz')):
        print('Subject: ' + str(subject.id) + ' NA.')
        return None
...
arg_parser.add_argument('--reg_algorithm', default='niftyreg', choices=['standard', 'bidir', 'niftyreg'])
