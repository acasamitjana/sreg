...
class FS_Nonlin(results_utils.LabelFusion):
    ...
    def compute_p_label(self, timepoints, image_list, svf_list, age_list, tp, subject, parameter_dict, regnet_model):
        ...
        for it_tp_flo, tp_flo in enumerate(timepoints[1:]):
            filepath = join(subject.results_dirs.get_dir('longitudinal_segmentation_st_freesurfer') + '_AFF', '100000_9',  tp_flo.id + '.post.nii.gz')
            ...
            if tp_flo.id == tp.id:
                ...
                voxMosaic_ref_resize = np.reshape(voxMosaic_ref[:3], (3,) + ref_shape)
                voxMosaic_ref_torch = voxMosaic_ref_resize.copy()
                voxMosaic_ref_torch = torch.from_numpy(voxMosaic_ref_torch[np.newaxis]).float().to(device)
                flow_torch = torch.from_numpy(flow[np.newaxis]).float().to(device)
                flow_resampled = interp_func(flow_torch, voxMosaic_ref_torch)  # , mode='nearest')
                flow_resampled = flow_resampled[0].cpu().detach().numpy()

                del svf, flow, voxMosaic_ref_torch, voxMosaic_ref_resize, flow_torch
            ...
            seg_onehot = np.transpose(np.asarray(proxyseg.dataobj), axes=[3, 0, 1, 2])
...
segmenter = FS_Nonlin(demo_dict=demo_dict, parameter_dict=parameter_dict, reg_algorithm=reg_algorithm,
                      temp_variance=temp_variance, spatial_variance=spatial_variance, device=device)

if num_cores == 1:
    for subject in subject_list:
        segmenter.label_fusion(subject, force_flag=force_flag, suffix='_AFF_S1')
