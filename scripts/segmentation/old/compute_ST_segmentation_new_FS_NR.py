...
def compute_p_label(timepoints, image_list, svf_list, age_list, tp, subject, parameter_dict, regnet_model):
    ...
    for it_tp_flo, tp_flo in enumerate(timepoints):
        ...
        if tp_flo.id == tp.id:
            ...
            im = torch.from_numpy(image_list[tp_flo.id][np.newaxis, np.newaxis]).float().to('cuda:0')
        ...
        #     nib.save(img, join(save_dir, str(t_var) + '_' + str(s_var), tp.id + '_' + tp_flo.id + '.image.nii.gz'))
        #     img = nib.Nifti1Image(np.argmax(seg_resampled, axis=-1).astype('uint16'), proxy.affine)
        #     nib.save(img, join(save_dir, str(t_var) + '_' + str(s_var), tp.id + '_' + tp_flo.id + '.seg.nii.gz'))
        #     pdb.set_trace()
...
def label_fusion(subject):
    ...
    results_dir_sbj = subject.results_dirs.get_dir('longitudinal_segmentation_st_freesurfer') + '_NR'
    ...
    for tp in timepoints_to_run:
        ...
        for t_var in temp_variance:
            for s_var in spatial_variance:
                ...
                if not exists(join(results_dir_sbj, str(t_var) + '_' + str(s_var))): os.makedirs(join(results_dir_sbj, str(t_var) + '_' + str(s_var)))
...
# sbj_212 = list(filter(lambda x: '_212' in x.id, subject_list))
...
'miriad_251_HC_M',
'miriad_244_AD_F', ]
subject_list = list(filter(lambda x: x.id in subject_id, subject_list))
