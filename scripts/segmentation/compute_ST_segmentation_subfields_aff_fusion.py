...
class FS_LinNonlin(results_utils.LabelFusionSubfields):
    ...
    def compute_1x1x1_inputs(self, tp, tp_flo, flow_dir, regnet_model, ref_shape):
        ...
        if tp_flo.id == tp.id:
            ...
            if self.reg_algorithm == 'niftyreg':
                ...
                flow = algorithm_utils.integrate_RegNet(svf, subject.image_shape_template, parameter_dict, device=device,
                                                        model=regnet_model)

            flow_resampled = def_utils.interpolate3DChannel(np.transpose(flow, (1, 2, 3, 0)),
                                                            voxMosaic_lin_ref[:3].T)
            # del svf, flow

            # Image
            voxMosaic_nl_ref = voxMosaic_lin_ref.copy()
            voxMosaic_nl_ref[0] += flow_resampled[..., 0]
            voxMosaic_nl_ref[1] += flow_resampled[..., 1]
            voxMosaic_nl_ref[2] += flow_resampled[..., 2]
        ...
        return rasMosaic_flo, np.transpose(flow_resampled, axes=(1, 0)).reshape((3,) + ref_shape)#

    def compute_1x1x1_inputs_lin(self, tp, tp_flo, ref_shape):
        results_dir_sbj = subject.results_dirs.get_dir('linear_st')
        cog = tp.get_cog()
        affine_matrix_ref = read_affine_matrix(join(results_dir_sbj, tp.id + '.aff'), full=True)
        affine_matrix_ref[:3, 3] += cog
    ...
    def compute_p_label(self, timepoints, image_list, flow_dir, age_list, tp, subject, parameter_dict, regnet_model):
        ...
        for it_tp_flo, tp_flo in enumerate(timepoints):
            ...
            if tp_flo.id == tp.id:
                ...
                rasDefField, flow = self.compute_1x1x1_inputs(tp, tp_flo, flow_dir, regnet_model, ref_shape)
                rasDefField_lin = self.compute_1x1x1_inputs_lin(tp, tp_flo, ref_shape)
                ...
                rasDefField_lin_resampled = self.resample_ras_space(rasDefField_lin, tp.vox2ras0, rasMosaic_ref)
                rasDefField_lin_resampled = rasDefField_lin_resampled.cpu().detach().numpy()

                ################################

                image_flo = image_list[tp_flo.id]
                im_resampled = self.resample_ras_space(image_flo[np.newaxis], tp_flo.vox2ras0, rasDefField_lin_resampled)
...
# idx = [it_s for it_s, sbj in enumerate(subject_list) if sbj.id == '099_S_1034'][0] # run 018_S_0142
# subject_list = subject_list[idx:]

print('[LONGITUDINAL SEGMENTATION] Start processing.')
spatial_variance = [3**2, 7**2, 11*2] # in grayscale
...
segmenterLH = FS_LinNonlin(filekey='subfields.lh', demo_dict=demo_dict, parameter_dict=parameter_dict,
                           reg_algorithm=reg_name, device=device, long_seg_algo=reg_algorithm,
                           temp_variance=temp_variance, spatial_variance=spatial_variance, suffix='_AFF_LH')
segmenterRH = FS_LinNonlin(filekey='subfields.rh', demo_dict=demo_dict, parameter_dict=parameter_dict,
                           reg_algorithm=reg_name, device=device, long_seg_algo=reg_algorithm,
                           temp_variance=temp_variance, spatial_variance=spatial_variance,  suffix='_AFF_RH')
