...
# project imports
from setup import DATA_DIR, NIFTY_REG_DIR, RESULTS_DIR
...
data_path_dict = {
    'Initial': join(RESULTS_DIR, 'Initial'),
    'Linear': join(RESULTS_DIR, 'Linear'),
...
if not exists(data_path_dict[reg_algorithm]):
    makedirs(data_path_dict[reg_algorithm])
...
for stain in ['NISSL','IHC']:
    ...
    if reg_algorithm == 'Initial' or reg_algorithm == 'Linear':
        pass
    elif exists(join(data_path_dict[reg_algorithm], stain + '.flow.tree.nii.gz')):
        proxy = nib.load(join(data_path_dict[reg_algorithm], stain + '.flow.tree.nii.gz'))
        total_flow = np.asarray(proxy.dataobj)
    elif exists(join(data_path_dict[reg_algorithm], 'MRI_' + stain + '.0N.flow.tree.nii.gz')):
        proxy = nib.load(join(data_path_dict[reg_algorithm], 'MRI_' + stain + '.0N.flow.tree.nii.gz'))
        total_flow = np.asarray(proxy.dataobj)
    else:
    ...
    for slice_filename, slice_info in slice_num_dict[stain].items():
        ...
        if reg_algorithm == 'Initial':
            mask = resize(M, final_shape, anti_aliasing=True)
            mask_volume[..., tree_pos] = mask
            for it_s in range(3):
                image = resize(H[..., it_s], final_shape, anti_aliasing=True)
            ...
            if reg_algorithm == 'Linear':
                field_x = affine_field[0]
                field_y = affine_field[1]
            ...
            mask = resize(mask, final_shape, anti_aliasing=True)
            mask_volume[..., tree_pos] = mask
            for it_s in range(3):
                image = deform2D(H[..., it_s], field, mode='bilinear')
                image = resize(image, final_shape, anti_aliasing=True)
                image_volume[..., tree_pos, it_s] = image
