...
from src.utils.deformation_utils import interpolate2D, deform2D
    ...
    def __call__(self, data, mask_flag, XX, YY, flow_x, flow_y, *args, **kwargs):
        ...
        for it_image, (image, m) in enumerate(zip(data, mask_flag)):
            if m:
                ...
                data_tf.append(interpolate2D(image, np.concatenate((x.reshape(1, -1), y.reshape(1, -1)), axis=-1)))
        return data_tf
...
class NonLinearDifferomorphismManyImages(NonLinearDeformationManyImages):
    ...
    def get_diffeomorphism(self, field_lowres_x, field_lowres_y, image_shape, reverse=False):
        ...
        for it_step in range(self.params.nstep):
            x = XX + flow_x
            y = YY + flow_y
            incx = interpolate2D(flow_x, np.concatenate((x.reshape(1, -1), y.reshape(1, -1)), axis=-1))
            incy = interpolate2D(flow_y, np.concatenate((x.reshape(1, -1), y.reshape(1, -1)), axis=-1))
...
class AffineTransformation(object):
    ...
    def __call__(self, data, mask_flag, *args, **kwargs):
        ...
        if mask_flag:
            ...
            data_tf = interpolate2D(data, np.concatenate((x.reshape(1, -1), y.reshape(1, -1)), axis=-1))
